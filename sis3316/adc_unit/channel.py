...
class Adc_channel(object):
    ...
    __slots__ = (
        ...
        "unit_noidx",
        "board",
    )  # Restrict attribute list (foolproof).
    ...
    def dac_offset(self, value):
        ...
        magic = [
            0x88F00011,  # 0x8-Write| 0x8-Set dcen/ref| 0xf-All|0x0|0x0|0x0|0x1 Standalone mode, Internal reference on | 0x1 - Misterious bit
            0x85000000
            + (chanmask << 20)
            + (0x1 << 4),  # Clear Code Register, 1 = Clears to Code 0x8000
            0x82000000
            + (chanmask << 20)
            + (value << 4),  # 0x8-Write| 0x2-Write to n, update all (soft LDAC)|...
        ]
        for spell in magic:
            ...
            usleep(
                10
            )  # Doc.: The logic needs approximately 7 usec to execute a command.
    ...
    @event_pickup_index.setter
    def event_pickup_index(self, value):
        if not (0 <= value <= 0xFFFF):
            raise ValueError("Pickup index should be in range0..65535.")
        reg = SIS3316_ADC_GRP(ENERGY_PICKUP_INDEX_REG, self.gid) + 4 * (self.cid % 4) # Group 1 Ch1: 0x10E0 ,Group 2 Ch6: 0x20E4
        self.board._set_field(reg, value, 16, 0xFFFF)
    ...
    def event_format_mask(self, value):
        ...
        if value & ~mask:
            raise ValueError(
                "A mask of the value is {0}. '{1}' given".format(hex(mask), value)
            )
    ...
    _auto_properties = {
        "addr_actual": Param(
            ...
            """ The actual sampling address for the given channel. points to 32-bit words.""",
        ),
        "addr_prev": Param(
            0xFFFFFF,
            0,
            PREVIOUS_BANK_SAMPLE_ADDRESS_REG,
            """ The stored next sampling address of the previous bank. It is the stop address + 1; points to 32-bit words.""",
        ),
        "fir_energy_peaking_time": Param(
            0xFFF,
            0,
            FIR_ENERGY_SETUP_REG,
            """ FIR Energy Peaking time [2, 4, 6, ..., 2044]""",
        ),
        "fir_energy_gap_time": Param(
            0x3FF,
            12,
            FIR_ENERGY_SETUP_REG,
            """ FIR Energy Gap time [2, 4, 6,..., 510]""",
        ),
        "fir_energy_tau_factor": Param(
            0x3F,
            24,
            FIR_ENERGY_SETUP_REG,
            """ FIR Energy filter Tau factor. Valid value 0 -> 63""",
        ),
        "fir_energy_tau_table": Param(
            0b11,
            30,
            FIR_ENERGY_SETUP_REG,
            """ FIR Energy filter Tau table. 0 [off],1,2,3.""",
        ),
    }
